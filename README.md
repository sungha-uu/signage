...
배포 폴더에는 두 실행 파일이 생성됩니다.

- `섹시한 꾼만두 메뉴판.exe`: 계절·전환 애니메이션이 없는 기본 버전.
- `섹시한 꾼만두 메뉴판 - 여름 애니메이션.exe`: 은은한 잎사귀 바람 효과와 화면 전환 애니메이션이 있는 버전.

두 EXE 모두 사이니지 모드로 실행하며 다음 동작을 자동으로 수행합니다.
...
완성된 두 EXE는 `dist/release/`에 생성됩니다.
