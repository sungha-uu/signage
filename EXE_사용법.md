...
배포 폴더에서 원하는 버전을 하나 골라 더블클릭합니다.

- `섹시한 꾼만두 메뉴판.exe`: 효과 없는 기본 정적 버전.
- `섹시한 꾼만두 메뉴판 - 여름 애니메이션.exe`: 여름 잎사귀·바람 효과와 부드러운 화면 전환이 있는 버전.

메뉴, 가격, 30초 전환, 영상 재생 방식은 두 버전이 동일합니다. 한 번에 한 버전만 실행합니다.
